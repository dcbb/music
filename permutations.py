...
def falling_permutations():
    ...
    pattern_len = 5
    chords = [Scale(C - 12, major).diatonic_chord(i - 1, pattern_len) for i in [2, 1, 0, -1, -2, -3, -4]]
    # another sequence
...
def falling_permutations_2():
    print('weird', flush=True)
    pattern_len = 5
    chords = cycle([Scale(C - 12, major).diatonic_chord(i - 1, pattern_len) for i in [2, 1, 0, -1, -2, -3, -4]])
...
def monotonic_permutations():
    ...
    chord = Scale(C - 12, dorian).diatonic_chord(0, 6)
